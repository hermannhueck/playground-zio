...
object TestingEffects extends App {
  ...
  // ------------------------------------------------------------
  printTextInLine("Environments: #provide, #access, #accessM")
  ...
  val conf: String = runtime.unsafeRun(
    configString.provide(Config("localhost", 9999))) tap println
  ...
  val tablesAndColumns
    : ZIO[DatabaseOps, Throwable, (List[String], List[String])] =
    for {
  ...
  val zioDB = tablesAndColumns.provide(new DatabaseOps {
    override def getTableNames: Task[List[String]] =
      Task.succeed(List("user_table", "user_table2"))

    override def getColumnNames(table: String): Task[List[String]] =
      Task.succeed(List("col00", "col01", "col02"))
  })
  val tableAndColumns
    : (List[String], List[String]) = runtime.unsafeRun(zioDB) tap println

  // ------------------------------------------------------------
  printTextInLine("Providing Environments: #provide")
  ...
  // ------------------------------------------------------------
  printTextInLine(
    s"""${dash(10, "=")} Environmental Effects ${dash(10, "=")}""")

  // ------------------------------------------------------------
  printTextInLine("Define the Service")
  ...
  // ------------------------------------------------------------
  printTextInLine("Provide Helpers")
  ...
  object db {
    ...
    def update(id: UserID,
               profile: UserProfile): ZIO[Database, Throwable, Unit] =
      ZIO.accessM(_.database.update(id, profile))
  }

  // ------------------------------------------------------------
  printTextInLine("Use the Service")
  ...
  // ------------------------------------------------------------
  printTextInLine("Implement Live Service")
  ...
  // ------------------------------------------------------------
  printTextInLine("Run the Database Effect")
  ...
  // ------------------------------------------------------------
  printTextInLine("Implement Test Service")
  ...
  // ------------------------------------------------------------
  printTextInLine("Test Database Code")
